...
echo "3. get_availability (doctor_id=$DOCTOR_ID)"
...
if [ -n "$SLOT_ID" ] && [ -n "$SERVICE_ID" ]; then
  echo "4. create_appointment_hold"
  HOLD=$(curl -sf -X POST -H "$AUTH" -H "Content-Type: application/json" \
    -d "{\"slot_id\":\"$SLOT_ID\",\"service_id\":\"$SERVICE_ID\",\"patient_name\":\"Test Patient\",\"patient_phone\":\"+5491112345678\",\"reason\":\"tooth pain\"}" \
    "$BASE/api/tools/smilecare/create_appointment_hold")
  echo "$HOLD" | python3 -m json.tool
  echo ""
  echo "All core tools OK."
else
  echo "Skipped hold — no slots or service_id."
fi
